...
try:
    ...
    import tkinter.tksimpledialog as tkSimpleDialog
...
# ControlFrameMod
#===============================================================================
class ControlFrameMod(CNCRibbon.PageLabelFrame):
    def __init__(self, master, app):
        CNCRibbon.PageLabelFrame.__init__(self, master, "Control", _("Control"), app)
        ...
        self.step1 = tkExtra.Combobox(self, width=6, background="White")
        self.step1.grid(row=row, column=4, columnspan=2, sticky=EW)
        self.step1.set(Utils.config.get("Control","step1"))
        self.step1.fill(map(float, Utils.config.get("Control","fsteplist").split()))
        tkExtra.Balloon.set(self.step1, _("Step for fine move operation"))
        self.addWidget(self.step1)
...
#            self.grid_anchor(CENTER)
    ...
    def incStep(self, event=None):
        if event is not None and not self.acceptKey(): return
        step, power = ControlFrameMod._stepPower(self.step.get())
        s = step+power
        zs = None
        if s<_LOWSTEP: s = _LOWSTEP
        elif s>_HIGHSTEP: s = _HIGHSTEP
    ...
    def incStepF(self, event=None):
        if event is not None and not self.acceptKey(): return
        step, power = ControlFrameMod._stepPower(self.step1.get())
    ...
    def incZStep(self, event=None):
        step, power = ControlFrameMod._stepPower(self.zstep.get())
    ...
    def setStep1(self, event=None):
        if event is not None and not self.acceptKey(): return
        self.setStep(self.step1.get(), self.zstep.get())
...
class ControlPage(CNCRibbon.Page):
    ...
    def register(self):
        ...
        self._register((ConnectionGroup, UserGroup, RunGroup),
            (DROFrame, ControlFrameMod, StateFrame))
